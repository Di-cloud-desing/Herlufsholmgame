...
var accesscode1="JOSEPHINE"
var accesscode2="DANNEBROG"
var accesscode3="FREDERIK"
...
{ 
  ...
  if(cena === 3){
    ...
    if(gameState===END){
      ...
      if(pontos>30){
          if(!somFail.isPlaying()){
          somFail.play()
        }
        textSize(20);
        fill("white")
        text("du overskred målet, prøv igen",120,339)
        setTimeout(() => {
        number0Group.destroyEach()
        number1Group.destroyEach()
        number5Group.destroyEach()
...
function perguntas(){
  textSize(15)
    fill ("black")
    text("sjienoeph",300,70)

    fill ("black")
    text("Hvem er 6.b dansk lære",300,90)

    fill ("black")
    text("nobgredan",300,220)

    fill ("black")
    text("hvad hedder det Danmarks flag",300,240)

    fill ("black")
    text("kerdifre",300,370)

    fill ("black")
    text("hvad hedder Danmarks konge i 2025",300,390)
}
